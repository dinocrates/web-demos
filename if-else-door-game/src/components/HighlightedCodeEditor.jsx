...
const typeWords = new Set(["int", "double", "string", "bool", "char", "String", "boolean", "void"]);
const keywordWords = new Set(["public", "private", "protected", "class", "static", "if", "else", "return"]);
const boolWords = new Set(["true", "false"]);
const variableNames = new Set(["blueKeys", "redKeys", "yellowKeys", "health", "coins"]);
...
function tokenClass(token) {
  ...
  if (actionNames.has(token)) return "text-sky-200";
...
export function HighlightedCodeEditor({ code, language, onCodeChange }) {
  ...
  return (
    <div className="retro-editor-frame relative h-[430px] overflow-hidden">
      <pre
        aria-hidden="true"
      ...
      <textarea
        ...
        aria-label={`Editable ${language} conditional rule`}
      />
    </div>
  );
}
